...
export default function FemaleProductDetails(props: any) {
  ...
  const { female, loading } = state;

  const [moreState, setMoreState] = useState({
    moreFemale: null,
    moreLoading: true,
  });
  const { moreFemale, moreLoading } = moreState;
  ...
  useEffect(() => {
    const fetchData = async () => {
      const female = await client.fetch(
        `*[_type == "female" && slug.current == $slug][0]`,
        { slug }
      );

      const moreFemale = await client.fetch(`*[_type == "female"]`);
      setState({ ...state, female: female, loading: false });
      setMoreState({
        ...moreState,
        moreFemale: moreFemale,
        moreLoading: false,
      });
    };
    fetchData();
  ...
  const addItemToBag = () => {
    ...
    const newFemale = { ...female, price: female.price * quantity };
    const product = { ...newFemale, size, color, quantity };
    // send product as an action to redux store.. The Bag Slice
    dispatch(addToBag(product));
    toast.success(`${female.name} added to cart successfully`);
  };
          ...
          {moreLoading ? (
            ...
            >
              ...
              <section className="w-full grid md:grid-cols-2 lg:grid-cols-3 gap-10 place-items-center place-content-center mt-10">
                {moreFemale.slice(0, 3).map((outfit: any) => (
                  <div key={outfit._id}>
                    <a href={`/shop/female/${outfit.slug.current}`}>
                      {/* image */}
                      <div className="">
                        <Image
